...
def _get_text_file(text_dir=TEXTDIR):
    file_list = glob.glob(f'{text_dir}/corpus_preprocessed.txt')
...
def train(prefix=PREFIX, vocab_size=VOCABSIZE, ctl_symbols=CTLSYMBOLS):
    ...
    command = f'--input={files} --model_prefix={prefix} --vocab_size={vocab_size} --user_defined_symbols={ctl_symbols} ' \
        f'--character_coverage=1.0 --add_dummy_prefix=false --pad_id=0 --unk_id=1 --bos_id=-1 --eos_id=-1'
